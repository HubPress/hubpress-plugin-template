...
function generateAuthors(opts) {
  ...
  _lodash2.default.each(authors, function (authorPosts, key) {
    ...
    authorObject.status = '';
    // ---
    returnedOpts = _paginationGenerator2.default.generate({ opts: returnedOpts.opts, posts: authorPosts, author: authorObject, template: template, path: 'author/' + key + '/' });
