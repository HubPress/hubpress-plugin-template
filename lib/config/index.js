...
ConfigManager.prototype.set = function (config) {
    ...
    assetHash = this._config.assetHash || crypto.createHash('md5').update(packageInfo.version + Date.now()).digest('hex').substring(0, 10);
...
ConfigManager.prototype.validate = function () {
    ...
    if (!config) {
        errors.logError(new Error('Cannot find the configuration for the current NODE_ENV'), 'NODE_ENV=' + envVal, 'Ensure your config.js has a section for the current NODE_ENV value and is formatted properly.');
